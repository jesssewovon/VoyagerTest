...
class VoyagerProjetsController extends VoyagerBaseController
{
    ...
    public function show(Request $request, $id)
    {
        ...
        $dataTypeContent_log = \App\Models\Log::where('projet_id', $dataTypeContent->id)->get();
        $dataType_log = Voyager::model('DataType')->where('slug', '=', 'logs')->first();
        //dd($data);
        $actions = [];
        if (!empty($dataTypeContent_log->first())) {
            foreach (Voyager::actions() as $action) {
                $action = new $action($dataType_log, $dataTypeContent_log->first());
        ...
        return Voyager::view($view, compact('dataType', 'dataTypeContent', 'isModelTranslatable', 'isSoftDeleted', 'dataTypeContent_log', 'dataType_log', 'actions'));
    }
